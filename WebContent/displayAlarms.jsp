...
<%@ page import="com.JAVA.Beans.Alarm" %>
    ...
    <div class="text-center">
        <div class="container" style="margin-top: 120px; font-weight: 800;">
            ...
            <div style="margin: 30px">
                <a href="addAlarm.jsp" class="btn btn-primary btn-lg" style="margin-left: 20px; margin-bottom: 20px;">Ajouter alarme</a>
            </div> 
        </div>
 <section id="hero" class="position-relative overflow-hidden d-flex align-items-center mb-5">
    <div class="container ">
        <div class="row align-items-center justify-content-center text-start ">
            <!--start leiba li ateawd list -->
            <div class="col-md-7" style="margin-top: 20px; background-color: #f5f5f5; border-radius: 20px;">
                <% 
                    List<Alarm> userAlarms = (List<Alarm>) request.getAttribute("userAlarms");
                    if (userAlarms != null && !userAlarms.isEmpty()) {
                        for (Alarm alarm : userAlarms) {
                %>
                <div class="mb-3" style="font-weight: bold; font-size: 20px; color: rgb(56, 56, 56); margin: 20px;">Titre de l'alarme: 
                    <%= alarm.getTitle() %>
                </div>
                <div class="mb-3" style="font-weight: bold; font-size: 20px; color: rgb(56, 56, 56); margin: 20px;">Date et heure:
                    <%= alarm.getTime() %>
                </div>

                <div class="mb-3 mt-4">
                    <% 
                        Set<String> repeatDays = alarm.getRepeatDays();
                        String[] allDays = {"Monday", "Tuesday", "Wednesday", "Thursday", "Friday", "Saturday", "Sunday"};
                        for (String day : allDays) {
                    %>
                    <input type="checkbox" name="repeatDays" value="<%= day %>" <% if (repeatDays.contains(day)) { %>checked<% } %>> <%= day %>
                    <% } // Closing bracket for the 'for' loop %>
                </div>
                <div class="d-flex">
                    <form action="alarmServlet" method="post">
                        <input type="hidden" name="action" value="delete">
                        <input type="hidden" name="alarmId" value="<%= alarm.getAlarmId() %>">
                        <div style="margin-left: 20px; margin-bottom: 20px">
                            <button type="submit"  value="Delete Alarm" class="btn btn-primary btn-lg " style="margin-top:20 !important ;">Supprimer</button>
                        </div>
                    </form>
                </div>
                <% } // Closing bracket for the 'for' loop %>
            ...
            <% } else { %>
            <p>No alarms found for the user.</p>
            <% } %>
            <!--end l3iba li ateawed-->
        </div>
    </div>
</section>
